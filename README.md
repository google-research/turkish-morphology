...
morphophonemic model, which takes a Turkish word and rewrites it into the
...
tape as the input and rewrites it to all possible morphological analysis
strings that can be generated from the hypotheses generated by the first level.
...
See [Interpreting Morphological Analysis Strings][12] section for a description
of such morphological analysis strings.
...
```
bazel run analyzer:print_analyses -- --word=[WORD_TO_PARSE]
```

This will morphologically parse the input word against the two-level
morphological analyzer and output a set of morphological analysis strings, as
such:

```
bazel run analyzer:print_analyses -- --word=geldiğinde
...
```
bazel run analyzer:print_analyses -- --word=foo
> 'foo' is not accepted as a Turkish word
```

## Interpreting Morphological Analysis Strings

An example output morphological analysis string is as follows;

**Input Word** (*evlerindekilerin* = those that belongs to ones in their
homes):

```
bazel run analyzer:print_analyses -- --word=evlerindekilerin
```
...
Morphological analysis strings can be decomposed into parts:

* **Inflectional groups:**

  Each morphological analysis string is composed of inflectional groups. An
  inflectional group is a sub-word span, and it is created by affixation of a
...
* **Analysis of morphemes:**
  ...
  `NDA` is the meta-morpheme in morpheme analysis string `+NDA[Case=Loc]`).
  ...
  feature value in morpheme analysis string `+NDA[Case=Loc]`).
...
* **Surface realization of inflections:**
  ...
  Some meta-morphemes are not realized in the surface form. These
  meta-morphemes do not correspond to a span of characters in the input
  word. For them we do not output any meta-morpheme strings in the morpheme
  analysis string (e.g. `+lArH[Possessive=P3pl]` and `+NDA[Case=Loc]`
  are realized in the surface form, thus they have explicit meta-morpheme
  strings `lArH` and `NDA` in their morpheme analysis string. However,
  `+[PersonNumber=A3sg]` and `+[Possessive=Pnon]` are not realized in the
  surface form, therefore only morphological feature category-value tags are
  output for them in their morpheme analysis strings).
...
* **Surface realization of derivations:**
  ...
  output non-empty meta-morphemes in the corresponding morpheme analysis
  strings of derivational morphemes. Meaning that no zero-derivations are
  allowed in the morphotactic model.
...
[1]: LICENSE
[2]: WORKSPACE
[3]: ./analyzer/src/lexicon/base
[4]: ./analyzer/src/lexicon/README.md
[5]: ./analyzer/src/morphotactics/model
[6]: ./analyzer/src/morphotactics/README.md
